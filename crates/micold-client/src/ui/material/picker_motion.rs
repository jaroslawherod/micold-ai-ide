...
// ---------------------------------------------------------------------------------------------
// BUG-001 (feature 022): an open select never stops asking for frames
// ---------------------------------------------------------------------------------------------

/// An open select asks for a frame for ever, so its list re-runs its entrance and blinks.
///
/// **Ignored because it fails.** It is committed rather than described because a reproduction that
/// runs is worth more than a paragraph, and because un-ignoring it is how the fix proves itself.
/// Reported by jaro against a `main` build, in the add-worktree dialog — not a showcase artefact.
///
...
/// | select pressed, then closed again | **yes** — so the trigger's ripple is not the cause |
/// | select open | **no** |
/// | select open, with `animated_menu` bypassed entirely | **no** — so the fade/scale is not the cause |
/// | search picker open | **yes** |
///
/// So it is specific to the select *while open*, and it is not the transition this feature added,
/// which was the obvious suspect and is wrong. The remaining difference from the search picker is
/// where `cdk::picker`'s `Visibility` track starts: the search picker is built already open, so its
/// track is constructed at its target and never animates, while the select is built closed and its
/// track has to climb. Re-running `layout` is *not* involved — delivering frames without a relayout
/// reproduces it identically.
#[test]
#[ignore = "BUG-001: reproduces the blink; un-ignore it with the fix"]
fn an_open_select_stops_asking_for_frames() {
    let r = roles();
    let mut h = Harness::new(
        Select::new(LABELS, Some(LABELS[0]), |t: &str| t.to_string(), r)
            .label("Type")
    ...
    h.clock += 1;
    let at = h.origin + FRAME * h.clock;
    let mut messages: Vec<String> = Vec::new();
    let mut shell = Shell::new(&mut messages);
    h.element.as_widget_mut().update(
        &mut h.tree,
        &Event::Window(iced::window::Event::RedrawRequested(at)),
        Layout::new(&h.node),
        mouse::Cursor::Unavailable,
    ...
    assert_eq!(
        ...
        "thirty frames after opening, the select still asks for another one. A track that never \
         settles re-runs its entrance every frame, which is the blink, and it burns a core while \
         it does it"
    );
}
