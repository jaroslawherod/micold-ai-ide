...
fn every_icon_tint_is_legible_on_its_surface() {
    for scheme in [ColorScheme::Light, ColorScheme::Dark] {
        ...
        let cases = [
            ...
            // A text/outlined button's leading glyph sits on whatever the button sits on, and the
            // button that has one is in the app bar — `surface`, like the label beside it.
            (IconSurface::AccentButton, r.surface),
...
fn icon_role_maps_each_surface_to_its_expected_foreground() {
    for scheme in [ColorScheme::Light, ColorScheme::Dark] {
        ...
        assert_eq!(icon_role(IconSurface::AccentButton, r), r.primary);
