...
fn worktree_menu_toggles_replaces_and_dismisses() {
    ...
    assert_eq!(state.worktree_menu_open.as_ref().unwrap().anchor, (120, 300));
    ...
    state.update(Message::WorktreeMenuToggled("feat-a".to_string(), (120, 300)));
    state.update(Message::WorktreeMenuToggled("feat-b".to_string(), (140, 610)));
    assert_eq!(open_dir(&state).as_deref(), Some("feat-b"));
    assert_eq!(state.worktree_menu_open.as_ref().unwrap().anchor, (140, 610));
    // Dismiss clears.
