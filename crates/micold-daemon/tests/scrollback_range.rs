...
fn a_range_returns_contiguous_lines_by_id() {
    ...
    let (lines, more) = framer.scrollback_range(&vt.term, frame.oldest_available, 10);
...
fn a_request_past_the_watermark_clamps_instead_of_erroring() {
    ...
    let (lines, _) = framer.scrollback_range(&vt.term, LineId(frame.oldest_available.0 - 1000), 5);
    ...
    let (empty, _) = framer.scrollback_range(&vt.term, LineId(frame.viewport_top.0 + 10_000), 5);
    assert!(
        empty.is_empty(),
...
fn a_lines_identity_is_immutable_under_new_output() {
    ...
    let (before, _) = framer.scrollback_range(&vt.term, anchor, 1);
    ...
    let (after, _) = framer.scrollback_range(&vt.term, anchor, 1);
