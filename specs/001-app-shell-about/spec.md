...
- **FR-014**: ~~When the About dialog opens, keyboard focus MUST move into the dialog; when it closes, focus MUST return to the main window.~~ (Superseded — spec/code alignment 2026-07-27: iced 0.13.4's `button` widget does not implement the framework's focusable operation — only `text_input`/`text_editor` do — so keyboard focus cannot be programmatically moved onto the Close button or back. The modal already blocks all interaction with the rest of the window while open (FR-013) and is dismissible via Close or Esc regardless of focus, so the trapping intent this FR aimed for is met without a focus change.) While the About dialog is open, it MUST remain the only interactive surface (FR-013), and dismissing it via Close or Esc MUST work regardless of keyboard focus state.
