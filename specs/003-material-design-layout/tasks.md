...
- [X] T015 [US1] Verify FR-014/FR-015/FR-016 via `quickstart.md` §3: button variants + interactive states are visibly distinct, and the layout reflows usably when the window is resized small (depends on T010–T014). *(Run 2026-08-21 on Linux, headless — `evidence/T015-T033-manual-walkthrough.md`. FR-014/FR-015 **pass**, measured by pixel rather than by eye: normal/hover/pressed/disabled fills differ, and focus was checked where it is reachable — text fields — because `tokens/state.rs` records that buttons cannot hold focus in this rendering stack (accepted fidelity gap #2, FR-043). FR-016 **fails**: narrowing the window drops the project name from every known-projects row, then the Forget label, then its icon, and finally overflows the card — filed as [BUG-002](bugs/BUG-002.md). The task is closed as *run*; the defect it found is tracked there.)*
