...
- **FR-022a**: System MUST guard against crash loops: after a bounded number of failed restarts within a short interval, System MUST stop auto-restarting that session and surface a clear error, leaving the session in the sidebar so the user can retry manually.
  - **Clarification (bugfix BUG-004, 2026-09-13)**: "within a short interval" is measured, not inferred from liveness. A respawned process counts as *recovered* — resetting the failed-restart count — only once it has stayed up for the **restart stability window** (`RESTART_STABLE_AFTER`, 10 seconds). A respawn that exits before then is a further failed restart. So a process that fails more slowly than one supervision tick (e.g. a `--resume` that exits after a second) still reaches the guard, while crashes further apart than the window never accumulate. The session reads `restarting…` until the window has passed.
...
**Bugfix**: 2026-09-13 — BUG-004 The crash-loop guard reset its count as soon as a respawned
process was observed alive (one 250 ms supervision tick), so a process that failed after a second
restarted forever. FR-022a clarified: a restart is recovered only after surviving a 10-second
stability window; no new requirement, no wire change.
