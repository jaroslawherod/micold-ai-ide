...
**Bugfix**: 2026-07-20 — BUG-002 Updated from bugfix patch. The `TerminalPane` event translation gains a sub-line scroll accumulator: `wheel_lines(delta, cell_height, &mut residual)` in `src/ui/material/terminal_pane.rs` maps both `ScrollDelta::Lines` (pass-through) and `ScrollDelta::Pixels` (divide by cell height) to whole lines, retaining the remainder in a new `PaneState.scroll_residual` across events and discarding it on direction change. Previously each event was quantized with `.round()` in isolation and dropped when it yielded 0, so pixel-precision touchpad scrolling never moved the viewport. It feeds both the local-scrollback and mouse-report branches (FR-013a). Covered by gui-gated tests in `terminal_pane.rs` (accumulation, remainder carry-over, direction reversal, line passthrough). Constitution check VI amended above — parity varies by windowing system, not only by OS.
...
**Bugfix**: 2026-08-09 — BUG-003 Updated from bugfix patch. The structure decision above — "the session runtime (`RuntimeTerminal`) is extended in place for dynamic size + scrollback" — described a terminal whose process was spawned in-process by code that could see the pane; feature 010 moved spawning into the session service, and nothing replaced the free invariant that gave a new process the right size. Sizing is now a two-part obligation: the client retains the pane's last `(cols, rows)` (`App::last_grid`, already recorded and previously unread) and states it *before* `SessionStart` in `view_and_start` / the `SessionSelected` arm; the service records a size per session whether or not it has a process and seeds every spawn from it (`010-daemon-session-persistence` FR-020a). FR-015's resize path is unchanged. Covered by a daemon test that starts a session after sizing it and a client test that asserts the message order over a real `Outbox`.

**Bugfix**: 2026-09-13 — BUG-004, BUG-005, BUG-006 Updated from bugfix patch. Three gaps in `src/ui/material/terminal_pane.rs`. (1) `KeyRouting::Copy` wrote `selectable_content()` unconditionally, so an empty selection overwrote the clipboard with `""`; it now writes only non-empty text, matching the auto-copy guard already on mouse release (FR-013c). (2) `TerminalPane.focused` was set by `ui::terminal::pane` but never read by `draw`, and `TermPalette::accent()` was unused; `draw` now strokes the 018 focus indicator (`state::FOCUS_RING_WIDTH`, `secondary` — `TermPalette::accent` moves from `primary` to `secondary`). The ring occupies a gutter of that width reserved on every side regardless of focus, so the cell origin, `grid_at` and `GridSizeReporter`'s `(cols, rows)` all subtract it — reserving it only while focused would send a resize to the process on every focus change, and drawing over cells would clip column 0. Pinned in `style_snapshot`. (3) Bracketed paste was an assumption with no task: a pure `keymap::paste_bytes(text, bracketed)` strips embedded `ESC[201~` to a fixed point and wraps, `GridCache::bracketed_paste()` reads the mode bit, and all three paste paths (chord, middle-click, context menu in `shell/clipboard.rs`) use them. The context-menu path samples the mode at request time, before the asynchronous clipboard read; a mode change inside that window is not a realistic race for a user gesture. Tests: gui-gated widget tests in `terminal_pane.rs` (recording clipboard, headless rasterisation) and pure tests in `tests/keymap.rs`.

**Bugfix**: 2026-09-14 — BUG-007 Updated from bugfix patch. R3 derived the terminal's default colours from `tokens::roles(scheme)` in the client only; since feature 010 the colour *queries* (`OSC 10/11/12`) are answered by the daemon's `DaemonListener`, whose `StandardPalette` folds the dynamic indices (256 foreground, 257 background, 258 cursor) to xterm entry 7, light grey — for both. Three changes. (1) The derivation moves to `micold-core` as `tokens::terminal_defaults(scheme)` (foreground `on_surface`, background `surface`), and `TermPalette::from_scheme` reads it, so drawing and answering share one definition. (2) A new fire-and-forget `ClientMsg::TerminalColorScheme { scheme }` (`ColorScheme` gains serde; `PROTOCOL_VERSION` 12 → 13) carries the client's resolved scheme; the client sends it from `on_connected` before the `Attach`/start sends, and from `update` whenever `State::color_scheme()` differs from the scheme last sent on this connection. (3) The daemon keeps one `TerminalColors` handle (an atomic scheme, light until told) in `DaemonState`, sets it from the new message, and passes it to every `DaemonListener`, which answers 256/258 with the foreground and 257 with the background and leaves indices below 256 to `StandardPalette`. Read at reply time, so a scheme change reaches sessions already running. Tests: a daemon test driving a real `DaemonListener` through `OSC 10/11/12` under both schemes and across a change, a server test that the message reaches the handle, and a client test over a real `Outbox` for the connect-time order and the change-only resend.

**Bugfix**: 2026-09-14 — BUG-008 Updated from bugfix patch. The client selection model (`src/selection.rs`, `Selection`) had no empty state: `Selection::start` sets both inclusive bounds to the pressed cell, so the `TerminalSelectStart` every locally handled left press publishes produced a one-cell selection that `TerminalPane::draw` highlighted and the release's auto-copy wrote. Three changes (FR-013e). (1) `TerminalPane` remembers the pressed viewport cell and publishes no `TerminalSelectUpdate` until the pointer enters another cell; after that every move publishes, including back onto the pressed cell. A local wheel scroll that moves the view, or a position outside the pane, ends that exemption; the focus gutter counts as the edge cell a press there lands on. The gate is in the pane, in screen cells, because anchors are `LineId`s and at the live bottom `row_line_id` advances with every line of output, so a same-cell jitter under streaming output would otherwise reach the model as a move. (2) A `Char` `Selection` is empty until its first `update`, whatever that update's anchor is. The pane's cell gate is the only place that decides what a drag is, and a second `LineId` comparison would disagree with it under streaming output. Until that update, `contains` is false and `text` is empty. `Word`/`Line` selections are non-empty from the press. (3) The left-release arm no longer writes `selectable_content()`, which reads the selection the pane was built with, from before the whole event batch, so a press and release delivered together (a touchpad tap, a synthetic click) copied the *previous* selection. It publishes a new `SessionMsg::TerminalSelectionReleased`, which the shell resolves with `selection::copy_request` on the current `app.selection` after the batch's `TerminalSelectStart`/`Update` have been applied. `TerminalCopyRequested` is not reused because its handler also closes the terminal context menu. Tests: unit tests in `selection.rs` and `tests/clipboard_request.rs`; pane tests for a same-cell move and for a press and release in one batch.
