...
- [X] T041 **(DEFERRED)** Verify build + full test suite pass on Linux, macOS, and Windows in CI (Principle VI), both `--no-default-features` and `--features gui` (SC-006). Linux verified locally; macOS + Windows postponed to CI. *(2026-08-20: satisfied by the three-OS CI matrix added in `10a1fe7` (2026-07-20) — `.github/workflows/ci.yml` builds the whole workspace and runs the render-free core suite plus the component gates on ubuntu/macos/windows for every code-affecting change, and has been green on all three since. Latest run: [32302430171](https://github.com/jaroslawherod/micold-ai-ide/actions/runs/32302430171). The full GUI suite and clippy stay Linux-only by design — that is the only runner with the iced system deps.)*
- [X] T042 **(DEFERRED)** Run `specs/006-real-terminal-emulator/quickstart.md` end-to-end (all 10 manual steps + SC checks, incl. SC-008 responsiveness under flood). Postponed — needs a manual GUI run on a display. *(2026-07-20: step 10 added covering BUG-001 auto-focus + BUG-002 scrolling, since neither is reachable from a unit test — `on_event` needs a GPU-backed renderer. Verified by the user on Wayland/GNOME: touchpad scrolling moves the viewport (10a.2), the `htop` mouse-report path scrolls from a touchpad (10c.12), sidebar-click focus precedence holds (10e.17), and the scrollbar shows while scrolled back and hides at the live bottom (10b.6-7). BUG-001 and BUG-002 are therefore confirmed fixed end-to-end; the remaining step-10 checks and steps 1-9 are still unrun.)* *(2026-08-25: **closed**. The rest of the quickstart was run headlessly per the repo's `visual-pass` skill — Xvfb `:83` + lavapipe, private `XDG_RUNTIME_DIR`/`XDG_DATA_HOME`, a throwaway project — and every step now carries a verdict in [`evidence/gui-pass-2026-08-25.md`](./evidence/gui-pass-2026-08-25.md). Steps 3, 4, 6, 7, 9 and §10b/10c/10d/10e/10f/10g pass; step 2's gating passes and its indicator clause fails; step 5's gestures pass and its bracketed-paste clause fails. Four defects filed: BUG-004 (empty-selection copy wipes the clipboard), BUG-005 (no focus indicator, FR-010), BUG-006 (paste is not bracketed, FR-013), and `010-daemon-session-persistence` BUG-014 (a Settings save clobbers the persisted theme). Four claims stay out of reach and are recorded as unrun rather than passed: SC-008 perceived latency on a software rasteriser, the sub-line touchpad flicks of §10a.3-4 and §10c.13 (`xdotool` emits discrete notches only), interrupting a *running* `claude` turn, and SC-006's non-Linux platforms — the last carried by T041's three-OS CI matrix.)*

  > **Partly run 2026-08-20** — Xvfb + lavapipe, per the repo's `visual-pass` skill:
  > [evidence/partial-gui-pass.md](./evidence/partial-gui-pass.md). **Step 1 passes** in full —
  > bold/italic/underline/reverse, 256-colour and truecolor all render, and the theme clause holds
  > (defaults follow the scheme, the ANSI palette does not move). **Step 9's validation clause
  > passes** — an out-of-range scrollback is refused with "Enter a number between 100 and 1000000."
  > and nothing is persisted. Steps 2–7 were not attempted; step 8 (SC-008 responsiveness) is out of
  > reach on a software rasteriser and should not be answered from this harness at all.
...
## Phase 14: Bugfix BUG-007 — a program asking for the background is told "light" in a dark pane

**Purpose**: Make the `OSC 10/11/12` replies the colours the pane paints (FR-003a, SC-014). The
...
- [ ] T074 [BUG-007] [U1] [U2] [U3] [U4] [U5] [U6] [U7] Failing daemon test in `crates/micold-daemon/tests/vt_color_queries.rs`: a
  ...
  the terminator the query used; set to light, the light roles; switched from dark to light on the
  same listener, the *next* query answers light. A default `TerminalColors` answers light, and
  `ESC]4;1;?BEL` still answers xterm red. To fail for the reported reason rather than to a compile
  error, add `TerminalColors` and the `DaemonListener::new` parameter first as a stub the listener
...
- [ ] T077 [BUG-007] [U10] Wire message: `ClientMsg::TerminalColorScheme { scheme: ColorScheme }` in
  ...
  it: `protocol_auth.rs`'s `the_protocol_version_is_twelve` (renamed) and `schema_hash.rs`'s
...
- [ ] T078 [BUG-007] [U11] [A1] [A2] Daemon wiring, test-first: a failing test in `crates/micold-daemon/tests/` that a
  ...
  `server.rs` arm that ignores the message. Then make it pass — `DaemonState` owns one `TerminalColors`, the `server.rs` arm sets it, and
  `PtySession::spawn` (which builds the `DaemonListener`, `supervisor.rs`) takes a clone through
...
- [ ] T079 [BUG-007] [U12] [U13] [U14] [U15] Client reporting, test-first in `crates/micold-client/src/shell/daemon_sync.rs`'s
  ...
  alone), a later message that changes the resolved scheme sends exactly one new report; a message
  that does not change it sends none; a reconnect reports
...
- [ ] T080 [BUG-007] [A3] Visual pass on a private Xvfb display (the `visual-pass` skill), real client and
  ...
  shown on the display, capture its output under a pty answered with the branch's dark `OSC 11` reply
  and record that its body-text SGRs are its dark palette. Record under `evidence/`.

**Checkpoint**: A background-adaptive program in a dark pane is told the pane is dark, and a query made
after the user switches to light is told light; `claude` started in either scheme is readable. A
`claude` already running across a switch keeps its palette (no DEC mode 2031 — spec Edge Cases).

**Bugfix**: 2026-09-14 — BUG-007 Updated from bugfix patch. Added Phase 14 (T074–T080). No task is
