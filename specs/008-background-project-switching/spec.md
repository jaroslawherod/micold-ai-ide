...
- **FR-005**: The switcher MUST list the known projects and allow the user to make any listed project active in a single selection, without opening the folder-browser dialog. *(Clarified by bugfix [BUG-002](./bugs/BUG-002.md): a selection the application accepts closes the switcher — the contract's "Panel closes." — so SC-002's two interactions leave nothing to dismiss. A selection it refuses leaves the panel open, so the user sees the unavailable indication that refusal raised.)*
- **FR-006**: The switcher MUST clearly indicate which project is currently active.
- **FR-006a**: The active marker MUST occupy a **reserved slot**: the space it takes is held on every row, so a row without the marker starts its label at the same x as the row with it. Otherwise the marker does two jobs at once — it says which project is active *and* it shifts one label sideways — and the second is the louder of the two, turning a quiet indicator into a broken-looking list. The type-ahead's result rows already work this way and are the precedent (`typeahead.rs`: "a marker that shifted the text sideways would make the selection the loudest thing on the row rather than the quietest"). *(Added after the switcher's rows became menu items, where the leading slot is optional per item.)*
- **FR-007**: The switcher MUST indicate, per listed project, whether that project currently has running background sessions.
- **FR-008**: The switcher MUST indicate projects whose folders are unavailable or missing, and MUST NOT silently activate an unavailable project. *(Clarified by bugfix [BUG-003](./bugs/BUG-003.md): the indication reflects each folder as it is when the switcher is **opened** — acceptance scenario 3's "When the user opens the switcher" — so opening it rescans availability. A folder that has gone is marked, and one that has come back is selectable again, without a relaunch.)*
...
**Bugfix**: 2026-09-13 — BUG-002 FR-005 clarified: an accepted switcher selection closes the panel; a refused one leaves it open. Tasks T036–T037.

**Bugfix**: 2026-09-13 — BUG-003 FR-008 clarified: opening the switcher rescans availability, so the unavailable indication is current in both directions. Tasks T038–T039. The body's "Known projects" list shares the flag and is refreshed by the same scan, but has no scan of its own.
