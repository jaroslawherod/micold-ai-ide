...
- [X] T056 [P] [US2] Re-run the delete section of `quickstart.md` manually: delete a worktree with a running session and confirm the sidebar updates with no error banner. (Attempted 2026-07-20 — inconclusive: the delete was reported as showing no error banner, but the only instance confirmed running at the time was the installed `/usr/bin` build of 2026-07-18, which predates this fix. Re-run against a build verified to come from this branch.) **Re-run 2026-08-20 — PASS**, against a binary verified by `strings` to come from this checkout, built with the daemon in one invocation and pinned inside the build lock. Sidebar updated, no error banner, the session's process stopped with no orphan, and the worktree *and* its branch gone from git. Evidence: [evidence/T056-delete-with-running-session.md](./evidence/T056-delete-with-running-session.md).
...
- [X] T057 [P] [US2] In `crates/micold-core/tests/` (new `worktree_leftovers.rs`): `remove_worktree_dir` returns no leftovers when the target is already absent (guards the BUG-001 regression), and returns the surviving paths when the directory cannot be emptied. Build the unremovable case without privilege by clearing write permission on a parent directory (`0o555`), which yields `EACCES` on unlink exactly as a foreign-owned file does; restore the mode in the test's teardown so the temp dir can be cleaned up (FR-023c, FR-023d). **Done**: `crates/micold-core/tests/worktree_leftovers.rs` — absent target, fully-removable directory, a directory blocked by a `0o555` parent (verified to name the blocking path), and the report cap.
- [X] T058 [P] [US2] In `crates/micold-daemon/tests/mutation_semantics.rs`: a `WorktreeDelete` whose directory cannot be fully removed still resolves to `OperationOk`, still archives the worktree's sessions, and carries the surviving paths in its reply — while a delete whose *git* step fails still resolves to `OperationError` with the sessions untouched (FR-023c cases 2 and 3). **Done**: `mutation_semantics::worktree_delete_blocked_by_an_unremovable_path_still_archives_and_reports` — asserts `OperationOk` with leftovers, that git no longer registers the worktree, and that the session is archived anyway. Verified non-vacuous by emulating the pre-fix hard failure, which fails it exactly as reported.

### Implementation

- [X] T059 [US2] Change `remove_worktree_dir` in `crates/micold-core/src/worktree.rs` to report what survived instead of propagating a bare errno: return the surviving paths (with the owning uid where the platform exposes it) rather than `io::Result<()>`. Observation only — never retry the removal while reporting. Walk shallowest-first and cap the list, since a blocked `build/` tree can hold tens of thousands of entries; do not descend into an entry already identified as foreign-owned, as its whole subtree shares one cause. Makes T057 green (FR-023d). **Done**: returns `Vec<Leftover>` (path + owning uid where the platform exposes it). Walks shallowest-first, does not descend into an already foreign-owned entry, uses `entry.file_type()` so a symlink cannot walk the reporter out of the tree, and caps at `LEFTOVER_REPORT_CAP`.
- [X] T060 [US2] In `crates/micold-daemon/src/server.rs`'s `WorktreeDelete` handler, stop `remove_worktree_dir` failing the operation. A surviving directory is partial success: archive the worktree's sessions, refresh the catalog, and reply `OperationOk` carrying the leftovers. Keep a git-step failure a hard `OperationError` with sessions untouched, preserving T053's original intent. Makes T058 green (FR-023c). **Done**: leftovers no longer fail the operation — sessions are archived, the catalog refreshed, and `OperationOk` carries them. A git-step failure remains a hard `OperationError` with sessions untouched.
- [X] T061 [US2] Carry the leftovers on the wire: add them to `OperationResult::WorktreeDeleted` (`crates/micold-core/src/protocol/messages.rs`) alongside `branch_delete_failed`, which is the existing precedent for a non-fatal secondary outcome. Wire-visible, so bump `PROTOCOL_VERSION` (`crates/micold-core/src/protocol/version.rs`) and record the reason beside the existing bump log. **Done**: `OperationResult::WorktreeDeleted::leftovers`; `PROTOCOL_VERSION` 3 → 4 with the reason recorded beside the existing bumps.
- [X] T062 [US2] Surface the notice in `crates/micold-client/src/main.rs` where `WorktreeDeleted`'s `branch_delete_failed` is already handled: name the surviving paths and their owner, and say the worktree itself was removed, so a partial success does not read as a failed delete (FR-023d). **Done**: the notice leads with the worktree having been removed, then names up to two surviving paths with their owning uid and a count of the rest.
- [X] T063 [P] [US2] Log the partial-success case in the daemon at WARN with the leftover paths and owners, matching the delete diagnostics added alongside BUG-002's investigation. A silent partial success is what made this bug invisible for as long as it was. **Done**: WARN naming each leftover path and owner; the fully-successful case stays INFO.
