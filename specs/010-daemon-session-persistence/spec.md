...
| SC-013 | `cargo test --workspace` green on all three platforms in CI, on every pull request | `.github/workflows/ci.yml`, `build + test` matrix. **What a green Windows column does *not* say** is recorded at T083 and split out as T144: `platform/windows.rs::terminate_process_tree` is an empty no-op, so the Windows *behaviours* named there have nothing to exercise. *Amended 2026-09-13 (T144)*: they are implemented and run by `micold-daemon/tests/windows_process_tree.rs` in a Windows-only step. **The observable is still narrower than the criterion**: `cargo test --workspace` runs in the Linux column only; macOS and Windows run `cargo test -p micold-core --all-targets`, the enumerated client gates, and (Windows) T144's target — no other daemon test runs off Linux. *Amended 2026-09-14 (#330)*: macOS now runs `cargo test -p micold-daemon` in its own step, so the Unix arms of `platform/unix.rs` (`waitid(WNOWAIT)`, the `kill(-pid)` group signal) run off Linux too. Doing so exposed only test-isolation faults, fixed there. Windows still runs no daemon test beyond T144's target |
