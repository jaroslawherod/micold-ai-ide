...
- [X] T083 CI gate: build + full test suite pass on Linux, macOS, and Windows before merge; exercise Windows job-object teardown, `0x03` interrupt (NOT via `cmd.exe`), and the inverted `portable-pty` `kill()` result (Principle VI, Risk 3). ~~**Blocked in this environment** (no macOS/Windows runners)~~ — superseded: the gate exists and runs.
  **Done (the gate) 2026-07-28**: `.github/workflows/ci.yml` runs `fmt`, `clippy -D warnings` on the
  render-free core *and* the workspace, and `build + test` across a
  `[ubuntu-latest, macos-latest, windows-latest]` matrix with `fail-fast: false`, on every pull
  request. Observed green on all three platforms on PR #43 (run 30335853202) — so the
  Principle VI cross-platform gate is real, enforced before merge, and no longer hypothetical. The
  earlier note's premise ("no macOS/Windows runners") was about the *local* environment and was never
  true of CI.
  **Not done, and the reason has changed** (⚠️ tracked in `bugs/BUG-008.md`): the three Windows
  *behaviours* this task also names are still unexercised, but not for want of a runner —
  `crates/micold-daemon/src/platform/windows.rs` is 8 lines and its `terminate_process_tree(_pid)` is
  an **empty no-op**. There is nothing for the Windows job to exercise. A green Windows column
  therefore says "the suite compiles and passes on Windows", which is worth having, and says nothing
