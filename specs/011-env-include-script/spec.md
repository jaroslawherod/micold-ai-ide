...
4. **Given** *(added — BUG-002)* the project directory contains its own directory-dependent `PATH` configuration (e.g. a version-manager config file such as `mise.toml`/`.tool-versions` that a version manager's shell hook keys off the current working directory), **When** a session is opened in that project directory, **Then** the AI CLI process's environment includes the same directory-specific `PATH` contribution that opening a regular-terminal shell in that same directory would show — not just whatever the app's own launch-directory environment happens to contain.
...
- *(added — BUG-002)* Two sessions are open in different project/worktree directories, each with its own version-manager configuration contributing a different `PATH`: each session's own directory MUST be used as the working directory for resolving its own script-sourced environment (FR-020), so one project's directory-specific `PATH` additions never leak into — or are missing from — another project's session.
...
- **FR-020** *(added — BUG-002)*: When resolving the include script for a given session (FR-005), the sourcing subprocess MUST run with its working directory set to that session's own project/worktree directory (not the app process's own launch directory), so that directory-dependent `PATH` contributions from a version manager (mise, asdf, nvm, pyenv, rbenv, etc.) configured for that specific project are captured — matching what a real interactive shell opened in that directory would produce (User Story 1's Acceptance Scenario 4, SC-001's version-manager `PATH` promise). The baseline resolution used for diffing (FR-005) MAY continue to run from a neutral, project-independent directory, since it exists only to compute what bash/PowerShell adds by default, not to reflect any specific project. The per-application-run caching in FR-007 MUST be keyed by resolution directory (not a single application-wide instance), so this does not reintroduce a sourcing cost on every session launch — only the first launch against a given, not-yet-cached directory pays it.
...
- **Resolved Environment Snapshot**: The in-memory (never persisted) set of name/value pairs captured from sourcing the configured script during the current app run. ~~A single shared snapshot is used by every session~~ **Superseded (BUG-002)**: one snapshot is now resolved and cached per resolution directory (FR-020), not as one application-wide instance — sessions launched in the same directory share that directory's cached snapshot, while sessions in different directories get independently resolved snapshots. Each cached snapshot is recomputed once per (application run, directory) pair and refreshed by either a Settings save or any session's manual restart control (FR-007); all cached snapshots are discarded when the app exits.
...
**Bugfix**: 2026-07-23 — BUG-002 The sourcing subprocess never had its working directory set to the session's project/worktree directory, so directory-dependent `PATH` contributions from a version manager (mise, asdf, nvm, pyenv, rbenv, etc.) were silently never captured for the AI CLI process — the single application-wide snapshot was always resolved from wherever the app process itself happened to start. FR-020 added (sourcing MUST use the session's own directory as the working directory); User Story 1 gained Acceptance Scenario 4 and a new Edge Case; the "Resolved Environment Snapshot" Key Entity is superseded from a single shared instance to one cached per resolution directory. See `bugs/BUG-002.md`.
