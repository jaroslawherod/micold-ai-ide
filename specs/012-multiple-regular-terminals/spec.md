...
- **FR-010a** (bugfix BUG-004): The per-instance restart affordance MUST be reachable — laid out at its full size and pressable — on **every** instance that offers it, including one that is not the active instance. Restarting a background instance without first selecting it is the whole point of addressing the restart message by instance id rather than to the attached process; an affordance laid out at zero width satisfies every structural claim about the control (it exists, it is conditioned correctly, it dispatches the right message) while satisfying none of FR-010.
...
**Bugfix**: 2026-08-18 — BUG-004 The tab that offers a restart cannot fit one. `TAB_WIDTH` gives a
