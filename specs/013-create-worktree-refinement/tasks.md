...
- [X] T032 [P] [US3] [U1] [U2] Failing test in `crates/micold-client/tests/` (alongside
  `overlay_dismissal_rules.rs`; written as `add_worktree_dismissal.rs`, asserting the rule through
  `registry::escape` and `app::on_escape`, which the key and the scrim consult): with the add-worktree form open and `status == Editing`, the
...
- [X] T033 [P] [US3] [U3] [U4] [U5] Failing test: `app::on_escape(state)` returns `None` while a create is in
...
- [X] T034 [P] [US3] [U6] [U7] [U8] [U9] [U10] [U11] Failing reducer tests: with the form closed (`state.worktree_form.form ==
...
- [X] T035 [US3] [U1] [U2] Make `AddWorktreeDialog::dismissal()` in
...
- [X] T036 [US3] [U3] [U4] Stop `app::on_escape`'s Settings fallback when a registered surface is open but
  ...
  T035.
- [X] T037 [US3] [U6] [U7] [U8] [U9] [U10] [U11] Route create outcomes to notifications when the form is closed:
...
- [X] T038 [P] [US3] `docs/user-guide/worktrees-and-sessions.md` "Creating a worktree": clicking
...
- [X] T039 [US3] [A3] Visual pass (`visual-pass` skill) against a repository with submodules: start a
  create, click the scrim and press Escape — dialog stays, progress still visible; press Cancel —
  dialog closes; a notification reports the outcome when the create finishes. Depends on T035–T037. *(Passed 2026-09-19 on
  a private Xvfb display against a repository whose submodule fetch was slowed to ~30 s: scrim
  click and Escape left the dialog and its progress up, Cancel closed it, and the snackbar read
  `Worktree "feat-vp-final" created.` Evidence: `evidence/BUG-001-0[1-5]-*.png`.)*

### Acceptance (outer loop — green before BUG-001 is complete)

- [X] T040 [US3] [A1] Outer-loop test in `crates/micold-client/src/main_tests.rs`: an `App` with a
  ...
  T035, T036.
- [X] T041 [US3] [A2] Outer-loop test in `crates/micold-client/src/main_tests.rs`: Cancel during the
