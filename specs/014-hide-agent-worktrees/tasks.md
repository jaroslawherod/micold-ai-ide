...
- [X] T032 [P] Sweep every string this feature adds — the chip label, the badge, and the user-guide section — confirming all say "agent" and none says "assistant" (spec § Terminology, contracts/sidebar-reveal-control.md § Terminology) *(2026-08-21: re-swept by T034's scenario 7a and it had **regressed**. The app is clean — chip `ui/sidebar.rs:272`, badge `ui/sidebar.rs:334`, no "assistant" anywhere in `micold-client` or `micold-core`. The user guide was not: `b74dad4` (feature 016's held-branch message) added two user-facing "assistant" strings to `docs/user-guide/worktrees-and-sessions.md`, one of them sending the reader to a control it declined to name the same way. Both now read "agent". A one-off sweep cannot hold this; only re-running 7a caught it.)*
...
- [X] T034 Execute the manual procedure in [quickstart.md](./quickstart.md) Part 2 against a scratch repo, including the post-run `git worktree list` / branch diffs that prove nothing on disk changed (SC-005, FR-008) *(2026-08-21: run headlessly — Xvfb + lavapipe, this branch's own pinned client/daemon pair — and recorded in [evidence/T034-manual-procedure.md](./evidence/T034-manual-procedure.md) with frames. Scenarios 1–10 PASS, including the two reset clauses (FR-010a launch, FR-010e project switch) and the post-run diffs, which are empty: the app pruned, removed, renamed and adopted nothing, and the orphan directory survives. **7a FAILED and was fixed** — see T032. 11 PARTIAL: the 50-worktree list renders complete with no visible stall, but SC-004 is a comparison against a pre-feature build and is recorded unmeasured rather than passed. Scenario 3's "unmoved" is the quickstart being tighter than the spec — the list sorts by directory name, so revealed rows interleave; no requirement asks otherwise.)*
- [X] T035 Confirm CI is green on Linux, macOS, and Windows (Constitution Principle VI, Cross-platform gate) *(2026-08-20: satisfied by the three-OS CI matrix added in `10a1fe7` (2026-07-20) — `.github/workflows/ci.yml` builds the whole workspace and runs the render-free core suite plus the component gates on ubuntu/macos/windows for every code-affecting change, and has been green on all three since. Latest run: [32302430171](https://github.com/jaroslawherod/micold-ai-ide/actions/runs/32302430171). The full GUI suite and clippy stay Linux-only by design — that is the only runner with the iced system deps.)*
