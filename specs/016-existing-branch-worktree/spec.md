...
- **FR-024**: ⚠️ **UNRESOLVED — blocked by the daemon migration.** Progress and outcome reporting for creation was to cover the reuse, overwrite, and remote-continuation paths, naming the step being performed. Feature 010 (T055) moved git into the daemon and retired the client's live progress stream, so the form now shows a single "Creating worktree…" indicator with no stage to name. The core still exposes `CreateStage::label(mode)` with the per-mode wording, so this becomes implementable again the moment stage information is carried on a daemon progress message. **Outcome** reporting is unaffected and does distinguish the paths (see FR-021's holder-naming errors). Needs a decision: re-plumb progress through the daemon, or drop the requirement.
