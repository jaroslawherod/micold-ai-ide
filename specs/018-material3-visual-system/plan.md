...
| A panel that is individually correct and points at nothing | Every check this feature owns asks whether a component matches a figure, or whether two components overlap. None asks whether a panel is **adjacent to the thing it was opened from**, which is what a context menu is for. The sidebar's worktree and session menus anchored at a constant, `SIDEBAR_MENU_ANCHOR = (24, 96)`, and passed everything: right width, right surface, clear of the app bar (SC-008c), inside the window. The user's complaint was that a right-click at the bottom of a long sidebar is answered at the top of it. FR-029d states the rule for the kind and requires the press point to be **carried by the gesture** — `cdk::ContextArea` already supplies it, and `TreeItem::on_right_press` takes a bare message beside it — so a menu cannot be positioned by a constant without deleting the parameter that would position it. SC-008f is the gate, in SC-008e's shape: read the set of context menus, fail on an anchor that is a constant. Found the hard way — BUG-008, and twice recorded before that (BUG-003's "adjacent risk" note and T107) by two passes that both asked only whether the panel fits. |
...
| Every component knows its own colours, and nothing knows what it is standing on | The colour gates this feature builds read the **token set**: FR-004's test is exhaustive over the pairs §1.3 enumerates, and a table of enumerated pairs holds only the compositions somebody thought of. A pairing made by **composition** — a `Button` pushed into a `row` inside a styled `container`, three files from anything that names a colour — is in no product either gate takes, and the style snapshot pins the container's fill and the button's colour, both correct, one at a time. So the connection banner's `Take over` shipped in `primary` on `error`. iced's inherited text colour is why it reads as an oversight and is not one: the banner's container sets `text_color = on_error` and the title and detail take it *because they have no opinion*, while a `Button` writes its own into `button::Style` from the role §7.3 assigns it unconditionally — the one child with an opinion is the one whose opinion is wrong here. The tonal system then makes the failure exact rather than approximate, for the reason it was adopted: same tone, same luminance, 1.00:1 light and 1.01:1 dark, with the `outline` border at 1.42:1 against a 3:1 threshold. FR-004a states the rule, FR-027b puts the foreground in the component (which 017's single-definition rule makes both cheap and mandatory — the call-site workaround is the duplicate FR-021/FR-027 forbid), and SC-008g is the gate, reading the composition pair from the functions the view calls rather than from a restated inventory. Found the hard way — BUG-009, and it is BUG-003's "every gate reads one component at a time" in colour rather than in geometry. |

| A gate's recorded scope is a claim about the rest, and nobody reads it that way | BUG-009's composition gate deliberately covered **accent** hosts only, and said so in a module doc naming the nine neutral-host findings it was skipping and the bug they were filed as. That note is accurate, prominent and useless as a guard: the excluded class stays excluded until somebody chooses to go back for it, while the gate's greenness reads — to every later change, and to the check's own author three commits on — as "compositions are covered". It is worse than an untested area, because an untested area does not report success. What made the exclusion look safe was a sentence: BUG-009's own report and `Host::neutral`'s doc comment both say `surface_variant` is inside §1.3's `primary` enumeration, so a button there "keeps §7.3's table". The table lists three hosts and `surface_variant` is not one of them, and has not been since the feature was split out. A scope note is a promise about code nobody is measuring; the only durable form of one is a failing check or a narrowed contract. Found the hard way — BUG-010. |
...
**Bugfix**: 2026-08-20 — BUG-008 added the "a panel that is individually correct and points at nothing" risk row above. No design in this plan was wrong; what it never asked is where a menu opened from a row belongs.

**Bugfix**: 2026-08-21 — BUG-009 added the "every component knows its own colours, and nothing knows what it is standing on" risk row above. No design in this plan was wrong, and no approach changes: the tonal derivation is what makes the defect diagnosable at all, and 017's single definition of a button is what makes the fix one edit. What the plan never asked is what a component's foreground becomes when the thing beneath it is not a neutral surface — every risk row before this one is about a *number*, and this is the first about a *colour*.

**Bugfix**: 2026-08-25 — BUG-010 added the "a gate's recorded scope is a claim about the rest" risk row above. No design in this plan was wrong; what it did not anticipate is that BUG-009's own remedy would carry the next defect in its exclusion note — the composition gate is right, its scope is recorded honestly, and the recording is what let nine measured violations sit in the tree for four days looking handled. The approach does not change: the filter comes out, the contract narrows to the hosts that survive a state layer, and no palette tone moves.

**Bugfix**: 2026-09-13 — BUG-011 qualified BUG-010's "no palette tone moves": a role with exactly one host cannot be narrowed, so dark `inverse_primary` moves from tone 40 to 30. The ramps are untouched and no accent moves; the approach does not change.

**Bugfix**: 2026-09-13 — BUG-013 is the "a gate's recorded scope is a claim about the rest" row once more, with the claim in the contract rather than a test: §5's "layers never sum" was true of the states when written and stopped being true of the screen when feature 027 wrapped every button in a focus-drawing widget. No approach changes and no tone moves — the wrapper draws the heaviest single layer and shows focus only when it came by keyboard (FR-022a).

**Bugfix**: 2026-09-14 — BUG-014 is the same rule broken from inside this spec: FR-024c said the ripple "composes with" the state layers, and it did. No approach changes and no tone moves — while a ripple runs, the surface beneath it yields its layer through the two style helpers every rippling style already composes with, and the ripple fades to the layer the surface resumes (FR-024h). The visible cost is that a hovered button shows its resting fill outside the circle while it grows.

**Bugfix**: 2026-09-14 — BUG-015 Updated from bugfix patch. No approach changes: the snackbar's action is laid out before its message, through the trailing-first layout `cdk::reflow::Reflow` already provides, which learns to size itself by its content inside a content-sized parent. It is the single-component-gate risk once more — the width-cap check laid a long message out with no action, so the one arrangement that broke was never measured.

**Bugfix**: 2026-09-17 — BUG-016 Updated from bugfix patch. No approach changes: the shared keyboard wrapper answers a key only when no earlier element captured it, which is what FR-022a's "a key the control answers" already meant. BUG-013's tests drove a button alone, so the tree never held a second element to take the key first. A second, order-independent part: `ui/cdk/keyboard_elsewhere.rs` wraps the window and runs iced's `unfocus` operation while `State::terminal_focused()` holds, before any key reaches a control.
