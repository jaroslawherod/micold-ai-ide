...
- [X] **2 — baseline, post-change** (T076a). Same machine, same profile, same scene as 1.
      ...
      Machine: AMD Ryzen 7 260 (16 threads) / Radeon 780M, Ubuntu 26.04, Wayland/GNOME, debug build
      Date: 2026-08-06
      Frame time: 300 frames — mean 0.86 ms, p95 1.12 ms, max 1.48 ms

      | run | mean | p95 | max |
      |-----|------|-----|-----|
      | 1 | 0.88 ms | 1.12 ms | 1.30 ms |
      | 2 | 0.84 ms | 1.11 ms | 1.50 ms |
      | 3 | 0.86 ms | 1.12 ms | 1.48 ms |

- [X] **3 — full scene, post-change** (T076a). Same machine, same profile, same build as 2.

      Machine: AMD Ryzen 7 260 (16 threads) / Radeon 780M, Ubuntu 26.04, Wayland/GNOME, debug build
      Date: 2026-08-06
      Frame time: 300 frames — mean 0.90 ms, p95 1.13 ms, max 1.50 ms

      | run | mean | p95 | max |
      |-----|------|-----|-----|
      | 1 | 0.94 ms | 1.15 ms | 1.50 ms |
      | 2 | 0.89 ms | 1.13 ms | 1.34 ms |
      | 3 | 0.86 ms | 1.12 ms | 1.39 ms |

      An earlier attempt at this figure came out **bimodal** — three runs near `p95` 1.20 ms and
      three near 0.73 ms, with baseline runs interleaved between them holding steady, so the machine
      was not the variable. The cause was a gap in the verification rather than in the scene:
      `Scene::check` ran until it passed and was never asked again, so the 300 counted frames were
      measured against whatever the window drifted into. T083 closed it — the structural elements are
      re-checked on every counted frame and a drifted run exits rather than reporting, and the ripple
      is held to a coverage fraction over the run because it is the one element that legitimately
      blinks between cycles. Both refusals were confirmed to fire before these figures were taken.
      Six consecutive runs then landed within 0.12 ms of each other.
...
- [X] **1 → 2 is the comparison that matters.** Like-for-like: same scene, same machine. A gap here
      ...
      **1 → 2: `p95` 1.07 ms → 1.12 ms, +0.05 ms (+4.7%).** Above this machine's ~0.02 ms noise
      floor, and the three post-change runs agree to within 0.01 ms, so it is a real cost rather
      ...
      **2 → 3: `p95` 1.12 ms → 1.13 ms.** Inside the run-to-run spread, so the ripple's
      *composition* cost is below what this measurement can resolve. That is the whole of the claim:
      §B8 states this figure is blind to draw cost, and the ripple's cost is almost entirely draw —
      the band tiling and the quads it fills all land after composition returns. Read 2 → 3 as "the
      ripple adds no measurable composition cost", and read nothing else into it.
