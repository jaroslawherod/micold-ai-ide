...
- **FR-039b**: The frame-time figure MUST be measured on a single **defined reference scene**, so the measurement is reproducible rather than dependent on whatever happened to be on screen. The scene is deliberately stress-weighted, exercising the specific costs this feature introduces — overlapping shadows, state layers and a live ripple — because a typical-usage scene would under-report a regression in exactly those additions. The scene is: **20 worktrees in the sidebar, the sidebar expanded, one running terminal session, a context menu open over a dialog, captured while a ripple is mid-animation.** The scene definition MUST live in `quickstart.md` beside the measurement step, so a later regression is comparable to the original figure.
...
- **SC-018**: On the defined reference scene, a frame-time figure is measured for both the pre-change and the post-change build on the same machine, and both figures are recorded so the comparison is reviewable. The measurement is reported for trend and does not gate the build (FR-039c); a regression against the pre-change figure is raised as a review finding.
