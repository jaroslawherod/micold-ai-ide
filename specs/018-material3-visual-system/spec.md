...
- **FR-045**: Material's 48×48dp minimum interactive target (§7.3) is not met by the four controls in the **worktree sidebar's header**, which keep the smaller `IconButton::compact()` size. The sidebar's dense 36dp row density exists to satisfy FR-011 and FR-026a, and in a ~260dp panel the two requirements cannot both hold: four 48dp controls leave the header title 60dp for a word that wants 73.5px, and `tests/layout_text_overflow.rs` fails on the title painting past its clip. Every icon button **outside** the sidebar takes the full 48dp, so the shortfall is bounded to one header rather than general. Closing it means widening the sidebar (costing terminal width on every screen), shrinking the header title's role (making it inconsistent with every other section heading), or removing one of the four controls (a feature removal) — each a product decision beyond a styling pass. This is the fifth accepted, documented fidelity gap.
