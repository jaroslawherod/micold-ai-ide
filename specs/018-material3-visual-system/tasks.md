...
- [~] T076a **Two of three figures captured.** Figure 3 (full scene) is not recorded: six runs came out bimodal — three near `p95` 1.20 ms and three near 0.73 ms — while baseline runs interleaved with them held at 1.18 ms, so the machine was not the variable. `Scene::check` runs only while the window is being composed and nothing re-verifies during the 300 counted frames, so a run whose dialog or context menu dismissed itself after composition measures a lighter scene under the full scene's name. See T083. On the same machine that produced T000z's figure, capture the two post-change measurements: the **baseline** scene, and the **full** scene with a ripple mid-animation. Record both in `quickstart.md` §B8 alongside the pre-change figure. Compare: baseline-before vs baseline-after is like-for-like, and any gap there is a regression in rendering this feature did not add; baseline-after vs full is this feature's own cost. Neither gates the build; a regression is a review finding (FR-039b, FR-039c, SC-018)
