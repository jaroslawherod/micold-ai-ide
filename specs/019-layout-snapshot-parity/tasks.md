...
Appended by `/speckit-converge` on 2026-08-05, after the feature's 41 tasks were closed and merged (`1cff9b3`). Each traces to an artifact obligation the code does not yet meet. Nothing here is a change of intent — the spec is unmodified.

- [X] T042 Register a covered state whose project is `Availability::Unavailable` per FR-008c, SC-004 (missing). FR-008c names three required layouts — no project open, **an unavailable project**, and a disconnected daemon — and only two are covered. `ui/shell.rs:119` branches on availability and renders `Button::filled("Unavailable", r)` where the available path renders a `labeled(Icon::OpenProject, …, "Open")` composite: different content, so different geometry, and that branch is never laid out. The reason it was missed is worth recording — `empty-project-without-worktrees` reads like the unavailable case and is not it, and `FakeScanner::default()` reports `available: true` for every covered state, so nothing anywhere resolves the other branch. Needs a scanner that answers false, or a project whose availability is set after construction. SC-004 claims this coverage explicitly

  **Done, 2026-08-05.** `error-project-unavailable`: one known project whose folder is gone, none active — the state a restart shows, and the honest form of it, since `Workspace::activate` refuses to open an unavailable project. The layout differs as predicted: `Button::filled("Unavailable")` where the available path renders an icon-plus-label composite.

  **What made this reachable was not adding a state, it was noticing the scaffold could not produce one.** `FakeScanner::default()` answers `is_available: true`, so every state built through `workspace_with` took the available branch *by construction* — there was no covered state anywhere that could have taken the other, and no amount of registering screens would have found it. The covered state sets `availability` directly rather than arranging for a scanner to say no, which keeps FR-016's one-place rule intact.

  Guarded by `a_covered_state_holds_an_unavailable_project`, verified failing before the state existed. Fixture: +72 lines, all new — the 605-line diff is purely additive, so registering a state moved nothing

- [X] T043 Exercise scroll-dependent geometry in at least one covered state per FR-011 (partial). Nothing in the fixture scrolls: in `main-shell-sidebar-expanded` the sidebar scrollable and its content are both 704.8px, and `a_fresh_tree_samples_at_rest` proves "every scrollable at the top" against `State::default()`, which has no overflow either. So the scroll half of FR-011 is asserted about a tree in which no element's geometry depends on scroll position — **the same shape as the overlay-pass gap**, where a pass ran on every state and recorded nothing, and looked exactly like a pass that found nothing (see T031). Give a covered state enough worktrees to overflow its viewport, and assert the offset is a sampled point rather than an assumption

  **Done, 2026-08-05.** `main-shell-sidebar-scrolled-to-top`: thirty worktrees, enough to push the sidebar's list 686.8px past the viewport showing it. Before it, **nothing in the fixture overflowed anything** — `a_fresh_tree_samples_at_rest` proved "every scrollable is at the top" over `State::default()`, where no element's geometry depends on scroll position at all. True, and about nothing.

  That is the overlay-pass failure again, arrived at independently: a property asserted everywhere is worth exactly as many places as it can fail in, and neither the code nor the fixture reports that number. Worth recording as a pattern rather than as two incidents.

  **The detector took three attempts and the two rejected ones are the interesting part.** "Any child taller than its parent" passed immediately — on the collapsed accordion's overhang (`CLIP_REVEALED`), which is not scrolling. Adding a positive-height guard still passed, on the overlay pass shrinking a root below the body beneath it. What actually distinguishes a scroll viewport is **exactly one child, taller than itself**, which is how `Scrollable` lays out. A guard that passes on the wrong evidence is worse than one that fails.

  The new state also fires the containment invariant, correctly — scroll content escaping its viewport is what scrolling *is*. Recorded as `SCROLL_CONTENT`, deliberately a **separate list** from `CLIP_REVEALED`: `Expand` overhangs to reveal progressively, a `Scrollable` because its content does not fit, and merging them would leave one list meaning "overhangs we tolerate", which is the shapeless exemption the module documentation argues against. Attribution proven, not assumed — `the_recorded_scroll_overflow_is_the_sidebar_list` shows the same node contained at three worktrees and escaping at thirty.

  Guarded by `a_covered_state_scrolls`, verified failing first. Fixture: +533 lines, all new

- [X] T044 Correct the two stale typography exclusions per FR-015, SC-007 (contradicts). `layout_snapshot.rs:14` and `contracts/layout-fixture.md:121` both still say the typography a user actually sees is out of scope "until feature 018 ships Roboto as the application typeface". It shipped; the gate measures against `assets/fonts/` directly, and `docs/development/layout-snapshot.md:32` and `:155` record the gap as closed. The check's own module doc — the first thing a reader of the check sees — now **understates** its coverage and contradicts the reference doc. Two documents disagreeing about what a gate covers is the failure the B2 amendment already had to correct once, and SC-007 requires the answer to come from the documentation alone
