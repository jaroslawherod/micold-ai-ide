...
- [ ] T021 [US2] Move the session helpers `sessions_in_worktree`, `active_sessions`, `switch_active`, `record_foreground`, `restore_after_activation`, `restore_foreground`, `arm_notice`, `note_background_restart`, `session_mut` from `crates/micold-client/src/app.rs:2014–2155` to `crates/micold-client/src/features/session.rs` (~142 lines), **plus `SelectKind`**, reassigned here from T017
