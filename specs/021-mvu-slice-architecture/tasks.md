...
- [ ] T031 [US1] Migrate the 7 ad-hoc popovers off their loose `State` fields onto the registry — `help_menu_open`, `project_switcher_open`, `sidebar_filter_open`, `worktree_menu_open`, `project_menu_open`, `terminal_context_menu`, `session_menu_open` in `crates/micold-client/src/app.rs` (FR-007) — **found at T029, decide before migrating: registering six of these as they stand is a behaviour change.** Escape does not close them today; the keyboard path (`on_escape` and its `ui/mod.rs` mirror) only ever asks about a modal or the filter panel, and no widget handles Escape — `cdk::overlay::Surface` observes `OutsideClick` only. But the core rule says a `NonModal` surface dismisses on `Trigger::Escape`, so a uniform `DismissalRules` gives all six an Escape close they have never had. FR-012 says preserve the existing priority, which does not authorise that. Either the six are registered and the change is argued for on its own terms (feature 017 called unified dismissal its one sanctioned behaviour change, and this looks like the same argument), or the gap is preserved — but not by a special case in `DismissalRules`, which FR-014 rules out. T029 registered only `sidebar_filter`, the one popover Escape reaches, so nothing was decided by accident
