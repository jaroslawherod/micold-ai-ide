...
| B4 — re-run 2026-08-20, reproduced and fixed | **PASS against the fixed build** — [evidence](./evidence/B4-fixed-arrival-scrolls-to-the-row.png). BUG-002 was first *reproduced*: the ingredient §B never names is a panel that is not at the top when you leave it. Scroll the 30-worktree project to the bottom, pass through a project whose sidebar fits, and come back — the reveal in the short project moves the list without the rendering stack reporting it (iced's `notify_viewport` publishes nothing when the content fits), so the arrival measures its row against `734` while the panel is at `0`, calls it visible and scrolls nothing: `drained: scroll_offset=734 -> no scroll, the row is already visible (FR-009)` followed by `the scrollable reports offset 0`, list pinned at the top, first and settled frames pixel-identical, marked row 1,968px down. With the fix (T068) the same sequence reads `scroll_offset=0 -> scrolling to 735` and the panel arrives at the bottom of the list with the deepest row expanded, marked and fully visible. Traced with `MICOLD_REVEAL_TRACE=1` (T066). Step 4, "repeat at both densities", still has no user-facing route — `SIDEBAR_DENSITY` is a compile-time constant |
