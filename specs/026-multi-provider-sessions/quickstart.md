...
5. **B7's wording gap.** An unavailable stored default opens the list of available CLIs and starts
   nothing, but never says the default is missing (FR-002, FR-004 scenario 4). Filed 2026-08-27 as
   [BUG-001](bugs/BUG-001.md) rather than fixed: `start_intent` returns the same `OfferChoice` for
   this press as for a deliberate "show me the list", so the reason is gone before anything can draw
   it, and closing it is a change to the intent type rather than a wording change.
