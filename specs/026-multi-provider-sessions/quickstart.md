...
Four, none of them in the paths §A covers. Each has its own task, and all four are fixed.
...
5. **B7's wording gap.** An unavailable stored default opens the list of available CLIs and starts
   nothing, but never says the default is missing (FR-002, FR-004 scenario 4). `start_intent`
   returned the same `OfferChoice` for this press as for a deliberate "show me the list", so the
   reason was gone before anything could draw it — a change to the intent type rather than a wording
   change, which is why it was filed as [BUG-001](bugs/BUG-001.md) rather than patched in place.
   Fixed 2026-08-27 (T090, T091): the offer carries the default it opened in place of, and the
   reducer says so once, when the list opens and only if that default is still missing.
