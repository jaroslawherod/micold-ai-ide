...
*(Bugfix BUG-001: mdBook's search is elasticlunr, which drops English stop words at index time
(elasticlunr-rs, inside mdBook) and at query time (`elasticlunr.min.js` in the browser). mdBook
exposes no setting for the list, and `pages.yml` installs mdBook unpinned. How the site keeps a
topic word such as "about" searchable (FR-026a) is settled by research §1a, and it has to hold for
the mdBook version a publication actually installs.)*
...
│   │                                 #   (BUG-002: title start-aligned and centred vertically; every
│   │                                 #   chrome icon is a Material Symbols glyph, no `{{fa}}` helper)
...
└── checks/
    ...
    └── page-checks.mjs               # WCAG 2.2 AA (axe-core) + the SC-001/SC-006 structural proxies
                                      #   + per-word search (BUG-001), app-bar geometry and chrome icons
                                      #   (BUG-002), rendered transitions are tokens (BUG-003)
...
**Bugfix**: 2026-09-16 — BUG-001 Updated from bugfix patch. Technical Context records elasticlunr's
stop-word filtering and defers the workaround to research §1a. `page-checks.mjs` gains a per-word
search assertion.
**Bugfix**: 2026-09-16 — BUG-002 Updated from bugfix patch. `index.hbs` draws the app-bar title's
placement and the chrome icons as the application does; `page-checks.mjs` asserts both.
**Bugfix**: 2026-09-16 — BUG-003 Updated from bugfix patch. mdBook's own transitions are overridden
with `--micold-motion-*`, and `page-checks.mjs` checks the rendered page's transitions, which the
`site.css` grep cannot see.
