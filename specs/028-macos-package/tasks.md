...
- [X] T057 Run `mise run test` and `for t in scripts/tests/*.test.sh; do "$t"; done`; confirm the macOS and Windows legs are green in CI (Principle VI) *(Local: 259 suites / 2563 tests green, all four shell suites pass, `cargo fmt --check` and `cargo clippy --workspace --all-targets -D warnings` clean. Two gates went red on the way and were fixed rather than exempted: `feature_write_isolation.rs` wanted an `OWNERS` entry for the new `install_location` field, and `release_publishes_complete_sets.rs` counted `publish` as an artifact job because its new comment names `gh release upload` -- the scan now drops comment lines and a fixture test holds that. **The first of those no longer applies.** Merging `main` brought feature 028-feature-encapsulation, which moved per-feature state onto per-feature `State` structs and made `OWNERS` self-declaring for them; `install_location` now lives on `features::window::State`, so it needs no entry and the table is untouched. The merge also needed `docs/SUMMARY.md` entries for the two pages this feature adds, which `page-set.test.sh` -- new from main's documentation-site feature -- requires of every page under `docs/`. **The macOS and Windows legs were confirmed after pushing**: CI run 34747999622 on `3da33207` (the third merge of `main`) went green on ubuntu, windows and macos, the macOS leg including the packaging step; its `docs check` failed on a link to a `daemon.md` heading main had renamed and on a Linux logout mechanism main had removed, both fixed in `94b946c7`, whose run 34748089953 is green on every build-and-test leg and the docs check. Getting there took one more fix the local gate had passed only because it ran before that merge: `root_state_is_shared.rs` wanted `window.install_location` in its `COMPONENT_LOCAL` table, since the field is read by the root to refuse a session and so cannot belong to the screen. The shell suites now number fourteen, not four, with main's documentation-site suites included; all pass.)*
- [X] T058 Run quickstart §A and §B and confirm every row passes (`specs/028-macos-package/quickstart.md`) *(§A: every row green on Linux -- the whole table is text and directory-entry assertions, which is what §A's own "what it cannot tell you" note says. §B: **not run**. Its rows are `codesign`, a real launch and a `d.sock` appearing; `mise run app` on Linux stages unsigned and cannot exercise any of them. It runs on the macOS leg of `ci.yml`, so it settled on the push that settled T001: run 34747999622 composed the bundle, `macos-bundle.sh` verified it with `codesign --verify --strict` and required `Signature=adhoc` ("signed (ad-hoc) and verified"), the launched binary was alive when `d.sock` appeared, and the step then terminated the client. Two rows are observed more weakly than their wording: "still running a few seconds later" is checked once, at the moment the endpoint appears (1.15 s here), and "no process left behind" is not asserted -- the step stops the client, not the daemon it spawned, which by design outlives the window.)*
- [ ] T059 Run quickstart §C on a real Mac — install, the block, the refusal from the disk image, a working session, permissions, lifecycle, the floor — and fill in the record table in `specs/028-macos-package/quickstart.md` *(Tracked separately in issue #317 — needs a real Mac. The spec closed 2026-09-14 without it.)*
