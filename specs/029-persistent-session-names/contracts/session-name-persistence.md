...
| C16.1 | `read_title` answers with the name that CLI **currently** holds for the conversation, not the first or the most recently written of several (FR-005, BUG-002). For `ClaudeProvider` that is the latest non-empty `{"type":"custom-title","customTitle":…}`, else the latest non-empty `{"type":"agent-name","agentName":…}`, else the latest non-empty `{"type":"ai-title","aiTitle":…}`. Kind ranks above position: `claude` re-emits the pre-rename `ai-title` on every turn after a `/rename`, so the last line written is not the current name. Unchanged for `PiProvider` and `CopilotProvider`, whose formats have no second name record. |
