...
(US2 recovery) are the tasks whose read is corrected, and `data-model.md`'s *AI CLI records* entity and its empty-name validation rule are updated to match, and T017's FR-005 gate tests the catalog's
replacement rather than the record the name came from, which is why it could not have caught this.

- [X] T031 Write the failing regression gate first in `crates/micold-core/tests/ai_cli_provider.rs`, over a temp `<config>/projects/<encoded cwd>/<id>.jsonl` like the tests already there: after a `/rename`, `ClaudeProvider::read_title` returns the `custom-title` and **not** the pre-rename `ai-title` that `claude` keeps re-emitting beside it — fixture in the real record order `custom-title`, `ai-title`, `agent-name`, repeated over later turns (BUG-002 *What `claude` writes after a `/rename`*). Red on `origin/main` with `left: Some("PR 284 macOS feature")`, `right: Some("Windows package")` per FR-005 / C16.1
- [X] T032 Extend the same file with the rest of C16.1, still failing first: a transcript with `agent-name` but no `custom-title` reads the `agent-name`; with neither kind renamed, `agent-name` and `ai-title` are resolved **by position** — an `ai-title` written after the last `agent-name` wins, which is what keeps a later `claude` that stops emitting `agent-name` correct; the latest `custom-title` wins however early it sits; an empty `customTitle`, `agentName` or `aiTitle` is not a name and falls through; a transcript with only `ai-title` is unchanged (the `the_title_and_the_label_are_read_from_the_same_file_and_never_mixed` gate from 032, `tests/ai_cli_provider.rs:388` — it must keep passing); and a read still never errors on unparsable lines per FR-005 / C16.1 / C17
- [X] T033 Implement it in `ClaudeProvider::parse_title` (`crates/micold-core/src/provider.rs:384`): one pass over the transcript keeping the latest non-empty `custom-title` and, separately, the latest non-empty `agent-name`-or-`ai-title` **by position**; answer the `custom-title` if there is one, else the positional winner. Only `custom-title` ranks above position — a rename is sticky in `claude`'s records, so the last line written is not the current name — and keeping the other two positional is what survives a `claude` that stops writing `agent-name` (C16.1). Update the doc comment, which today says "the latest `ai-title` record". `PiProvider` and `CopilotProvider` are untouched per FR-005 / FR-006 / C16.1
- [X] T034 Extend *The name on a session row* in `docs/user-guide/worktrees-and-sessions.md` ("The newest name wins"): if you rename a conversation in the CLI yourself, that is the name the row keeps — the CLI's own earlier name does not come back when the app reads the conversation's records again. Verify with `mise run site-check` per Constitution VII / FR-005
- [X] T035 Run `mise run gate`, and confirm the BUG-002 reproduction is fixed end to end: the real transcript `c4cc3dca-36e3-4480-b76f-27ad4584181c.jsonl` now reads `"Windows package"` through `read_title`, and `crates/micold-daemon/tests/session_name_recovery.rs` still passes unchanged — the recovery pass's shape, cost and precedence are untouched (C14–C20, 032 C6.3/C6.6)
