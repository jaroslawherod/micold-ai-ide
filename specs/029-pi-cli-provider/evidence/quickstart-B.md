...
| B.6 `/name` and the fallback chain | **Partial. See finding 1.** `/name my task` recorded `session_info` `name: "my task"`, and the row changed within seconds, but to `π - my task - demo` (row 3). Before the first message the row read `π - demo`, not the neutral placeholder. After it, the row still read `π - demo`, not the first message. The chain holds only for rows the app is not running (B.8). |
...
1. **A running Pi session's row shows Pi's terminal title, not the name Pi recorded (FR-011).**
