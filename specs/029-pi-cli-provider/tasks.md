...
- [X] T019 [US1] Implement `PiProvider::is_available()` in `crates/micold-core/src/provider.rs` through the existing `resolves_on_path("pi")` — no spawn, no version read (FR-003, FR-003a, SC-006a)
