...
icons-only states, and the changes it makes to `NavigationDrawer`. Every clause names the
...
every sample, excluding the drawer's swap to its strip and any stretch where its width is floored at
its strip's (SC-002). That floor: the drawer's node is never narrower than its rail while it slides
(D20; U23). That swap: a closing drawer shows its rail once its panel is no wider than the rail, and
at `CLOSED` when the rail is no wider than the handle, so the swap moves nothing beside it and no
still panel is left on screen (D21; U24).
...
| Pointer | hover and press reach only the drawn form, and only while the cursor is inside the row's bounds; a press the drawn form takes keeps that form drawn (moved onto the line like any drawn form) until the release, lift or lost event, which reaches it and so publishes a click as at rest, and the row then takes the form its width asks for, invalidating the layout if that is another form; parked forms receive only the frame's redraw and the release/lift/lost/cursor-left events, with the cursor unavailable; their messages and captures are dropped, their redraw requests and invalidations kept, so no press is left held and no ripple frozen in a form that parked | FR-009, FR-011 |
| Overlays | forwarded from the drawn form only; none constructed | `one_overlay_implementation.rs` |

## 6. Not contracted

- The worktree sidebar's collapsed strip itself (unchanged).
