...
| ″ `the_keyboard_stays_on_the_collapse_control…` | focus on the control, no reachable control off screen, the reachable count equal to rest's, and the section's focusables in rest order (by line and height), on every frame including each row's form change | `RowSlide` before the focus handoff (focus lost at the icons-only swap); mutant: `operate` reaching parked forms (six controls off screen, at rest too, so the count alone could not see it) | FR-007, FR-008, SC-003 |
...
| ″ `the_state_flips_on_the_press`, `a_section_chosen_mid_slide_is_shown_at_once`, `an_exit_mid_slide_closes_settings` | `collapsed` changed before the first frame; mid-slide selection, Save and Cancel (Settings' only exits) take effect on the frame they happen (`settings_draft` becomes `None`) | mutant: the flag held until the slide settles, or `Rail::update` withholding events while animating | FR-010 |
...
| ″ `rows_keep_their_height_and_icons_their_line` | every row's height is its rest height; per-frame icon step within the SC-007 bound and toward the target | T020's single-rendering rail (103 px icon step on the first frame); the WIP's harness predates `Surface`, so it was not rerun there | FR-013, FR-014, SC-007 |
| ″ `selecting_mid_slide_moves_only_the_changed_icons` | changed rows' icons move by `12 · f` (±0.5) at once; unchanged rows' not at all; the slide settles | passed once `RowSlide` existed; mutant: `RowSlide` caching `x_labelled` from its first layout | FR-014 exception |
...
| `section_list.rs` unit tests (in-crate mount) `a_row_with_no_icon_keeps_its_chip` | chip inside the row and the row inside the rail on every frame; never empty; never taller than the taller rest state | mutant: a no-icon row laid out at 272 like the others | FR-015, spec Edge Cases |
...
| ″ `a_press_held_across_a_form_change_is_a_click` | a row pressed at `f` just above 0.001 stays in its pressed form until released; the release over the row after 0.001 publishes `SectionShown`, the row then takes its icons-only form, and after expanding a bare release publishes nothing | `RowSlide` holding no press (form swaps under the press), or never letting go | FR-009 |
| ″ `a_press_let_go_off_its_row_still_changes_form` | the same press let go off the row publishes nothing and invalidates the layout | the release not invalidating the layout | FR-009, FR-011 |
...
| ″ `a_ripple_is_cut_off_at_the_rail` | after a click on **Collapse**, the strip past the rail on the control's line draws exactly as a flag-collapsed twin's on every mid-slide frame | `RowSlide::draw` handing the form the whole viewport (found by the M2 visual pass) | FR-009, FR-013 |
...
| `tests/gates/rail_icons_align.rs` | collapsed icons on one axis, each inside the rail (the second clause added in M2: with the slots reordered every glyph measured was parked at −8.5e37, where f32 put them all on one "axis") | mutant: `icons_only` not the last child slot | FR-006 |
...
the settings view. Names and *Fails on* match `tdd/test-list.md` and `tdd/cycle-log.md` (T043).
