...
## Phase 9: Follow-up — a press held across a form change

- [X] T053 [US2] [U21, U26] A press is a click across the slide's last form change (user request after close; ledger *Follow-ups not done*): `RowSlide` keeps the form that captured a press drawn until the release, lift or lost event, then invalidates the layout if its width asks for another form (research R5, contract §5 *Pointer*). Tests: `a_press_held_as_its_form_parks_is_released` becomes `a_press_held_across_a_form_change_is_a_click` (the release over the row publishes `SectionShown`, red first on the dropped click), and new `a_press_let_go_off_its_row_still_changes_form`; mutants *never hold*, *never let go* and *no relayout on let go* each fail (cycle-log cycle 14)
