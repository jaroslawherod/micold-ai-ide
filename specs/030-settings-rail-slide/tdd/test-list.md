...
| U21 | A row pressed just above `f = 0.001` keeps its pressed form drawn until the press is let go: released over the row after the rail passes 0.001 it publishes the row's message, then the row takes its icons-only form, and on expanding a bare release over it publishes nothing | FR-009 | example (mutant) | DONE | `crates/micold-client/tests/settings_rail_motion.rs::a_press_held_across_a_form_change_is_a_click` (was `a_press_held_as_its_form_parks_is_released`, which held the click dropped; T053) |
| U26 | A press held across the form change and let go off its row publishes nothing and invalidates the layout, so the row takes the form its width asks for with the rail at rest | FR-009, FR-011 | example (mutant) | DONE | `crates/micold-client/tests/settings_rail_motion.rs::a_press_let_go_off_its_row_still_changes_form` (T053) |
