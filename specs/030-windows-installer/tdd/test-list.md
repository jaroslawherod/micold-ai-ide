...
| U48 | cargo is invoked as `build --release --locked -p micold-client --bin micold-ai-ide -p micold-daemon --bin micold-daemon --target <triple>` | FR-002, FR-016 | example | any | DONE | `scripts/tests/windows-installer.test.sh` "builds the app and the daemon for <triple>" |
