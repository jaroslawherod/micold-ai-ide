...
use crate::worktree::{
    BranchCandidate, BranchSituation, CreateMode, CreateProgressEvent, CreateStage, Worktree,
    WorktreeStatus,
};
...
pub struct WorktreeForm {
    ...
    /// The mode the in-flight create is running under, so progress labels can name the actual
    /// step (FR-024). `NewBranch` until a resolution says otherwise.
    pub mode: CreateMode,
}
        ...
        match message {
            ...
            Message::AddWorktreeOverwriteConfirmed => {
                ...
                if let Some(form) = &mut self.worktree_form {
                    if matches!(form.resolution, ResolutionState::ConfirmingOverwrite { .. }) {
                        ...
                        form.mode = CreateMode::Overwrite;
                    }
                }
            }
            ...
            Message::AddWorktreeResolutionCancelled => {
                if let Some(form) = &mut self.worktree_form {
                    ...
                    form.mode = CreateMode::NewBranch;
                }
            }
