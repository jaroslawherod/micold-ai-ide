...
use micold_ai_ide::worktree::{preflight, BlockReason, BranchSituation, CreateError, CreateMode};
...
fn every_mode_situation_pair_matches_the_contract_and_incompatible_ones_never_mutate() {
    ...
    for mode in &modes {
        for situation in &situations {
            ...
            let names = micold_ai_ide::naming::DerivedNames {
                dir_name: "feat-login".to_string(),
                branch: "feat/login".to_string(),
            };
            let before = snapshot(&git, &repo());

            let result = micold_ai_ide::worktree::create_worktree(
                &git,
                &repo(),
                &target(),
                &names,
...
fn a_situation_that_changed_since_the_prompt_is_reported_as_such() {
    ...
    let err = micold_ai_ide::worktree::create_worktree(
        &git,
        &repo(),
        &target(),
        &names,
...
fn tracking_the_second_remote_creates_a_branch_tracking_that_remote() {
    ...
    micold_ai_ide::worktree::create_worktree(
        &git,
        &repo(),
        &target(),
        &names,
