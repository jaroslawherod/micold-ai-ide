...
//! `State::system_scheme`; `Message::SystemThemeChanged` now carries the raw detection outcome
...
fn a_successful_detection_updates_the_scheme() {
    ...
    state.update(Message::SystemThemeChanged(Ok(SystemScheme::Dark)));
...
fn a_transient_detection_failure_keeps_the_last_known_scheme() {
    ...
    state.update(Message::SystemThemeChanged(Err(())));
...
fn a_genuine_unspecified_reading_is_applied_like_any_other_successful_read() {
    ...
    state.update(Message::SystemThemeChanged(Ok(SystemScheme::Unspecified)));
